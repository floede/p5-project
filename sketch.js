...
let strokeWidth = 44;
let ifMargin = false;
let marginVal = 0.5 * dimFactor + strokeWidth + (size % dimFactor);
...
function draw() {
  ...
  for (let hPos = margin; hPos < size - margin + 5; hPos += dimFactor) {
    for (
  ...
  // Draw vertical lines
  for (let vPos = margin; vPos < size - margin + 5; vPos += dimFactor) {
    for (
      let pos = margin + getRandomInt() * dimFactor;
      pos < size - margin;
      pos += 2 * dimFactor
